...
object CsvExporter {
    fun records(records: List<FinancialRecord>, currency: CurrencyOption): String = buildString {
        ...
        records.forEach { r ->
            appendLine(listOf(r.id, r.type.label, r.title, r.counterparty, r.category, r.amountMinor.asMoney(currency), r.paidMinor.asMoney(currency), r.remainingMinor.asMoney(currency), r.dateEpochDay, r.dueDateEpochDay ?: "", "").joinToString(",") { it.csv() })
        }
    }
